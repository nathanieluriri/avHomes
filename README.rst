...
``/admin/tutorials`` teaches the everyday jobs: one short silent video per job,
with the captions burned in, and a "Try it now" that opens the job's real screen
with ``?spotlight=<id>`` so a guided walkthrough can take over there. The
catalogue is ``src/lib/admin/tutorials.ts``: each tutorial lists every domain its
walkthrough touches in ``requires``, and a member sees it only when their role
holds all of them. Progress (watched, done) is theirs alone, stored in
``tutorial_progress``, and is not written to the audit trail.

The media lives in ``public/tutorials``: ``<id>.mp4`` (1080p, no audio) and
``<id>.jpg``, its 1280 by 720 poster. The burned-in captions also exist as text
in ``src/lib/admin/tutorial-steps.ts``, generated by ``videos/_tools/steps.cjs``
from each video's timeline, and the player shows them as a step list that
follows the video. Regenerate that file whenever a video is re-cut.

The recordings are made from the real console, not a mock-up of it. The scripts
in ``videos/_tools`` drive the UI in headless Chrome against the local demo
stack: ``scripts/demo-server.mjs`` runs ``next dev`` on port 3300 against a
local ``mongod`` on port 27018, and sets those values itself (no env file
needed; they take precedence over ``.env.local``). The recorder does not send
``/api/admin`` requests to the API: it answers them from an in-memory mock,
seeded from that database when a run starts, so nothing a recording does is
written back.

.. code-block:: bash

   node scripts/demo-server.mjs                # the demo stack, in its own terminal
   . videos/_tools/env.sh                      # Chrome, puppeteer-core and ffmpeg paths
   node videos/_tools/record.cjs <scenario>    # raw footage, see videos/_tools/scenarios
   node videos/_tools/cut.cjs <name>           # the edit and its caption timeline
   node videos/_tools/compose.cjs <name>       # the titled, captioned composition
