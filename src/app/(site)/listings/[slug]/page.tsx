...
export async function generateMetadata({
  ...
  const where = property.city && !property.location.includes(property.city)
    ...
    : property.location;
  /*
   * An estate's bedroom column is its largest available option, so "3 bed estate
   * land" would describe no single thing on the page. It is described by its options.
   */
  const estateLine = summary
    ? `Estate Land in ${where}. ${summary.count} ${summary.count === 1 ? "option" : "options"}${
        summary.fromMinor > 0 ? ` from ${formatPriceShort(summary.fromMinor, property)}` : ""
      }.`
    : null;
  const description = (
    estateLine
      ? [estateLine, property.tagline]
      : [
          `${deal} at ${price}.`,
   ...
   */
  const headline = `${[property.title, property.city].filter(Boolean).join(", ")}${
    estate ? `, from ${price}` : ` at ${price}`
  }`;
