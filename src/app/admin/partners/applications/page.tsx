...
export default function PartnerApplicationsPage() {
  ...
  const [url, setUrl] = useState<{ id: string; url: string; partnerId?: string } | null>(null);
        ...
        <div className="space-y-2">
          {applications.map((row) => (
            <ApplicationCard
              ...
              partnerId={url?.id === row.id ? url.partnerId : undefined}
              onDecided={(link, partnerId) => {
                if (link) setUrl({ id: row.id, url: link, partnerId });
                state.reload();
              }}
            />
          ))}
...
function ApplicationCard({
  ...
  showUrl: string | null;
  partnerId?: string;
  onDecided: (url: string | null, partnerId?: string) => void;
}) {
  ...
  async function decide(approve: boolean) {
    ...
    setError(null);
    try {
      const res = await api.post<{ url: string | null; partnerId?: string }>(
        `/admin/applications/${row.id}/decide`,
        { approve, reason: reason.trim() },
      );
      onDecided(res.url, res.partnerId);
    } catch (err) {
